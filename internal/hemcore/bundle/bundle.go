...
import (
	...
	"github.com/qyinm/hem/internal/hemcore/tar"
...
func maxBundleBytes() uint64 {
	if value := os.Getenv("HEM_MAX_BUNDLE_BYTES"); value != "" {
		var parsed uint64
...
func maxContentBytes() uint64 {
	if value := os.Getenv("HEM_MAX_CONTENT_BYTES"); value != "" {
		var parsed uint64
		if _, err := fmt.Sscan(value, &parsed); err == nil {
			return parsed
		}
...
// Export writes a snapshot to a .hem bundle archive.
func Export(options *types.BundleExportOptions) (*types.BundleExportResult, error) {
	...
	entries = append(entries, dirEntry(".hem/", now))
	entries = append(entries, fileEntry(".hem/format-version", []byte(formatVersion+"\n"), now))
	...
	entries = append(entries, fileEntry(".hem/manifest.json", manifestBytes, now))
	...
	checksumsEntry := fileEntry(".hem/checksums.json", checksumsContent, now)
...
// Import reads a .hem bundle and optionally applies content.
func Import(options *types.BundleImportOptions) (*types.BundleImportResult, error) {
	...
	formatEntry := findEntry(entries, ".hem/format-version")
	if formatEntry == nil {
		return nil, &Error{Message: "Invalid bundle: missing .hem/format-version"}
	}
	...
	manifestEntry := findEntry(entries, ".hem/manifest.json")
	if manifestEntry == nil {
		return nil, &Error{Message: "Invalid bundle: missing .hem/manifest.json"}
	}
	...
	if checksumsEntry := findEntry(entries, ".hem/checksums.json"); checksumsEntry != nil {
...
func Verify(options *types.BundleVerifyOptions) (*types.BundleVerifyResult, error) {
	...
	if formatEntry := findEntry(entries, ".hem/format-version"); formatEntry != nil {
		...
		errors = append(errors, "Invalid bundle: missing .hem/format-version")
	}

	manifestEntry := findEntry(entries, ".hem/manifest.json")
	if manifestEntry == nil {
		errors = append(errors, "Invalid bundle: missing .hem/manifest.json")
	...
	if checksumsEntry := findEntry(entries, ".hem/checksums.json"); checksumsEntry != nil {
		...
		errors = append(errors, "Invalid bundle: missing .hem/checksums.json")
	}
...
func resolveSignatureKey(explicit *string) *string {
	...
	if value := os.Getenv("HEM_BUNDLE_KEY"); value != "" {
		return &value
	}
	return nil
}
...
func canonicalSignaturePayload(entries []types.TarEntry, manifest *types.BundleManifest) []byte {
	...
	for _, entry := range entries {
		if entry.Path == ".hem/manifest.json" || entry.Path == ".hem/checksums.json" {
			continue
		}
...
func updateManifestEntry(entries *[]types.TarEntry, manifest *types.BundleManifest) error {
	...
	for i := range *entries {
		if (*entries)[i].Path == ".hem/manifest.json" {
