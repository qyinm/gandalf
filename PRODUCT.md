...
### 한 줄 요약
**AI 코딩 에이전트 환경을 Docker image처럼 스냅샷으로 저장하고 어디서든 동일하게 복원하는 CLI**
...
이 번들을 다른 머신에서 import 하면 **동일한 에이전트 환경**이 복원됩니다 —
...
- `.env` 값은 기본적으로 key name만 수집 (full content는 --include-content + --experimental)
- 절대 symlink를 따라가지 않음
- 번들은 기본 metadata-only (파일 내용 포함은 명시적 opt-in)
...
```bash
# 머신 A: 내가 세팅한 완벽한 에이전트 환경을 번들로 저장
...
snaptailor snapshot create --name baseline --project .
# ... 시간이 흐른 뒤 ...
