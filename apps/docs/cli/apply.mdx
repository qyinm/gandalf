...
sidebarTitle: "apply"
description: "Apply gandalf.toml to local Codex and Claude Code setups with Smart Merge. A pre-apply safety snapshot is created automatically for easy rollback."
---
...
With `--project-only`, apply skips user-home writes and Smart-Merges repository agent files (`.mcp.json`, `.cursor/mcp.json`, `.codex/config.toml`) using uninterpolated `${ENV}` templates so `gandalf check --project-only` / the CI Action can pass.
