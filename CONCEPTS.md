...
### Project-Only Scope
The CI/automation scope that reads and writes repository agent files without touching user-home configs. `gandalf check --project-only` (and the GitHub Action) compare `gandalf.toml` with committed `.mcp.json`, `.cursor/mcp.json`, `.codex/config.toml`, skill sources, and `[env_template]`. `gandalf apply --project-only` Smart-Merges those project MCP files and keeps `${ENV}` templates uninterpolated so the CI check can pass. Default `gandalf apply` remains user-home Smart Merge.
