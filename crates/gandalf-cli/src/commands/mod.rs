...
pub(crate) struct CommonOptions {
    ...
    #[arg(long, env = "HOME")]
    home: Option<PathBuf>,
    /// Gandalf store directory (defaults to ~/.gandalf or $GANDALF_STORE)
    #[arg(long, env = "GANDALF_STORE")]
    ...
    S: AsRef<str>,
{
    ...
    let cli = match Cli::try_parse_from(std::iter::once("gandalf".to_string()).chain(args)) {
        Ok(cli) => cli,
        Err(error) => {
            let _ = error.print();
...
fn execute_snapshot_create(args: SnapshotCreateArgs) -> i32 {
    ...
    if !args.metadata_only && !content_backed_codex_user {
        return write_error(&SnapError {
            code: "GANDALF_METADATA_ONLY_REQUIRED".to_string(),
    ...
    let state = match capture_current_state(&capture_runtime, &args.name) {
        ...
        Err(error) => {
            return write_error(&SnapError {
                code: "GANDALF_SNAPSHOT_CAPTURE_FAILED".to_string(),
    ...
    if let Err(error) = write_snapshot(
        ...
        return write_error(&SnapError {
            code: "GANDALF_SNAPSHOT_WRITE_FAILED".to_string(),
...
fn execute_snapshot_list(common: CommonOptions) -> i32 {
    ...
    let names = match list_snapshots(Path::new(&runtime.store_dir), runtime.agent) {
        ...
        Err(error) => {
            return write_error(&SnapError {
                code: "GANDALF_SNAPSHOT_LIST_FAILED".to_string(),
...
fn execute_snapshot_show(args: SnapshotShowArgs) -> i32 {
    ...
    let snapshot = match read_snapshot(
        ...
        Err(error) => {
            return write_error(&SnapError {
                code: "GANDALF_SNAPSHOT_NOT_FOUND".to_string(),
                problem: format!("Snapshot \"{}\" not found.", args.name),
                cause: error.to_string(),
                fix: "Run `gandalf snapshot list` to see available snapshots.".to_string(),
                path: None,
            });
        }
    };
...
fn execute_restore(args: RestoreArgs) -> i32 {
    ...
    if let Err(error) = ensure_store(Path::new(&runtime.store_dir)) {
        return write_error(&SnapError {
            code: "GANDALF_STORE_INIT_FAILED".to_string(),
    ...
    let exists = match snapshot_exists(
        ...
        Err(error) => {
            return write_error(&SnapError {
                code: "GANDALF_SNAPSHOT_LOOKUP_FAILED".to_string(),
    ...
    let plan = match build_restore_plan(&restore_options) {
        ...
        Err(error) => {
            return write_error(&SnapError {
                code: "GANDALF_RESTORE_PLAN_FAILED".to_string(),
    ...
    let experimental = args.experimental || std::env::var("GANDALF_EXPERIMENTAL").is_ok();
    if !experimental {
        return write_error(&SnapError {
            code: "GANDALF_EXPERIMENTAL_REQUIRED".to_string(),
            problem: "Restore --apply requires --experimental.".to_string(),
            cause: "--apply was used without GANDALF_EXPERIMENTAL=1 or --experimental.".to_string(),
            fix: "Set GANDALF_EXPERIMENTAL=1 or pass --experimental to enable experimental features.".to_string(),
            path: None,
        });
    }
    ...
    let plan_json = match serde_json::to_string_pretty(&plan) {
        ...
        Err(error) => {
            return write_error(&SnapError {
                code: "GANDALF_RESTORE_SERIALIZE_FAILED".to_string(),
    ...
    if !parsed.errors.is_empty() {
        return write_error(&SnapError {
            code: "GANDALF_RESTORE_PARSE_ERROR".to_string(),
        ...
        Some(value) => {
            if !VALID_AGENTS.contains(&value.as_str()) {
                return Err(SnapError {
                    code: "GANDALF_INVALID_AGENT".to_string(),
    ...
    let scope = match &common.scope {
        Some(value) => {
            if !VALID_SCOPES.contains(&value.as_str()) {
                return Err(SnapError {
                    code: "GANDALF_INVALID_SCOPE".to_string(),
...
) -> Result<gandalf_core::Snapshot, SnapError> {
    if reference == "current" {
        let state = capture_current_state(runtime, "current").map_err(|error| SnapError {
            code: "GANDALF_CURRENT_STATE_FAILED".to_string(),
            problem: "Failed to capture current state.".to_string(),
            cause: error.to_string(),
            fix: "Verify project and store paths are accessible.".to_string(),
            path: None,
...
fn render_scan_text(scan: &ScanResult) -> String {
    ...
    lines.push("  gandalf snapshot create --name baseline --agent codex --scope user --project .".to_string());
...
fn format_restore_plan_preview(plan: &RestorePlan) -> String {
    let mut lines = vec![
        "gandalf restore dry-run".to_string(),
        String::new(),
...
fn format_restore_plan_item(item: &gandalf_core::RestorePlanItem, index: usize) -> String {
    let mut fields = vec![
        format!(
            "{}. {} {} at {}",
            index,
...
fn format_risk_summary(risk_summary: &gandalf_core::RiskSummary) -> String {
    let counts = [
...
pub(super) fn write_json<T: serde::Serialize>(value: &T) -> i32 {
    match serde_json::to_string_pretty(value) {
        ...
        Err(error) => write_error(&SnapError {
            code: "GANDALF_JSON_SERIALIZE_FAILED".to_string(),
