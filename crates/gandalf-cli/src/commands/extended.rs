...
pub enum BundleCommand {
    /// Export a snapshot to a .gandalf bundle
    Export(BundleExportArgs),
    /// Import a .gandalf bundle
...
pub fn execute_doctor(args: DoctorArgs) -> i32 {
    ...
    let scan = scan_project(&gandalf_core::ScanOptions {
    ...
    lines.extend(format_readiness_summary_lines(
        &report,
        &gandalf_core::ReadinessFormatOptions {
            max_items: Some(10),
            include_fixes: true,
    ...
    let exit = report
        .items
        .iter()
        .any(|item| item.category == gandalf_core::ReadinessCategory::Blocked);
...
pub fn execute_report(args: ReportArgs) -> i32 {
    ...
    let scan = if args.reference == "current" {
        ...
        scan_project(&gandalf_core::ScanOptions {
            project_path: runtime.project_path.clone(),
            home_dir: runtime.home_dir.clone(),
            store_dir: runtime.store_dir.clone(),
            explain: None,
    ...
    if let Some(out) = &args.out {
        if let Err(error) = fs::write(out, &markdown) {
            return write_error(&SnapError {
                code: "GANDALF_REPORT_WRITE_FAILED".to_string(),
...
fn execute_timeline_list(args: TimelineListArgs) -> i32 {
    ...
    let entries = match list_timeline_entries(
        ...
        Err(error) => {
            return write_error(&SnapError {
                code: "GANDALF_TIMELINE_LIST_FAILED".to_string(),
    ...
    let mut lines = vec!["gandalf timeline".to_string(), String::new()];
...
fn execute_timeline_show(args: TimelineRefArgs) -> i32 {
    ...
    let entry = match find_timeline_entry_for_ref(&runtime, &args.reference, &mut Vec::new()) {
        ...
        Ok(None) => {
            return write_error(&SnapError {
                code: "GANDALF_TIMELINE_NOT_FOUND".to_string(),
                ...
                fix: "Run `gandalf timeline list` to see available entries.".to_string(),
                path: None,
            });
        }
        Err(error) => return write_error(&error),
...
fn execute_timeline_undo(args: TimelineUndoArgs) -> i32 {
    if !args.dry_run {
        return write_error(&SnapError {
            code: "GANDALF_TIMELINE_UNDO_DRY_RUN_REQUIRED".to_string(),
            problem: "Timeline undo requires --dry-run.".to_string(),
            cause: "`timeline undo` was called without `--dry-run`.".to_string(),
            fix: "Run `gandalf timeline undo <id> --dry-run --json`.".to_string(),
            path: None,
        });
    }
    let runtime = match resolve_runtime(&args.common) {
    ...
    let plan = match build_timeline_undo_plan(
        ...
        Err(error) => {
            return write_error(&SnapError {
                code: "GANDALF_TIMELINE_UNDO_FAILED".to_string(),
...
fn execute_bundle_export(args: BundleExportArgs) -> i32 {
    ...
    let result = match bundle_export(&BundleExportOptions {
        ...
        Err(error) => {
            return write_error(&SnapError {
                code: "GANDALF_BUNDLE_EXPORT_FAILED".to_string(),
...
fn execute_bundle_import(args: BundleImportArgs) -> i32 {
    ...
    if args.apply_content && !args.experimental && std::env::var("GANDALF_EXPERIMENTAL").is_err() {
        return write_error(&SnapError {
            code: "GANDALF_EXPERIMENTAL_REQUIRED".to_string(),
            problem: "Bundle content apply requires --experimental.".to_string(),
            cause: "--apply-content was used without GANDALF_EXPERIMENTAL or --experimental.".to_string(),
            fix: "Set GANDALF_EXPERIMENTAL=1 or pass --experimental.".to_string(),
            path: None,
        });
    }
    let result = match bundle_import(&BundleImportOptions {
        ...
        Err(error) => {
            return write_error(&SnapError {
                code: "GANDALF_BUNDLE_IMPORT_FAILED".to_string(),
...
fn execute_bundle_inspect(args: BundlePathArgs) -> i32 {
    let result = match bundle_inspect(&args.bundle_path) {
        ...
        Err(error) => {
            return write_error(&SnapError {
                code: "GANDALF_BUNDLE_INSPECT_FAILED".to_string(),
...
fn execute_bundle_verify(args: BundleVerifyArgs) -> i32 {
    let result = match bundle_verify(&BundleVerifyOptions {
        ...
        Err(error) => {
            return write_error(&SnapError {
                code: "GANDALF_BUNDLE_VERIFY_FAILED".to_string(),
...
fn find_timeline_entry_for_ref(
    runtime: &gandalf_core::RuntimeOptions,
    reference: &str,
    corrupt_events: &mut Vec<gandalf_core::TimelineCorruptEvent>,
) -> Result<Option<gandalf_core::TimelineEntry>, SnapError> {
    gandalf_core::find_timeline_entry(
        Path::new(&runtime.store_dir),
        reference,
        TimelineListOptions {
            agent: runtime.agent,
    ...
    .map_err(|error| SnapError {
        code: "GANDALF_TIMELINE_LOOKUP_FAILED".to_string(),
...
fn render_timeline_undo_text(plan: &TimelineUndoPlan) -> String {
    let mut lines = vec![
        format!("gandalf timeline undo (dry-run): {}", plan.title),
