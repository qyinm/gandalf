//! `.gandalf` bundle export, import, inspect, and verify.
...
fn max_bundle_bytes() -> u64 {
    std::env::var("GANDALF_MAX_BUNDLE_BYTES")
        .ok()
...
fn max_content_bytes() -> u64 {
    std::env::var("GANDALF_MAX_CONTENT_BYTES")
        .ok()
        .and_then(|value| value.parse().ok())
...
pub fn bundle_export(options: &BundleExportOptions) -> BundleResult<BundleExportResult> {
    ...
    entries.push(dir_entry(".gandalf/", now));
    ...
    let checksums_entry = TarEntry {
        path: ".gandalf/checksums.json".to_string(),
    ...
    for entry in entries {
        ...
        if final_entries
            .last()
            .is_some_and(|entry| entry.path == ".gandalf/manifest.json")
            && !checksums_inserted
        {
...
pub fn bundle_import(options: &BundleImportOptions) -> BundleResult<BundleImportResult> {
    ...
    let format_entry = entries
        .iter()
        .find(|entry| entry.path == ".gandalf/format-version")
        .ok_or_else(|| BundleError::Message("Invalid bundle: missing .gandalf/format-version".into()))?;
    ...
    let manifest_entry = entries
        .iter()
        .find(|entry| entry.path == ".gandalf/manifest.json")
        .ok_or_else(|| BundleError::Message("Invalid bundle: missing .gandalf/manifest.json".into()))?;
    ...
    if let Some(checksums_entry) = entries.iter().find(|entry| entry.path == ".gandalf/checksums.json")
    {
...
pub fn bundle_inspect(bundle_path: &str) -> BundleResult<BundleInspectResult> {
    ...
    let checksums = entries
        .iter()
        .find(|entry| entry.path == ".gandalf/checksums.json")
...
pub fn bundle_verify(options: &BundleVerifyOptions) -> BundleResult<BundleVerifyResult> {
    ...
    if let Some(format_entry) = entries.iter().find(|entry| entry.path == ".gandalf/format-version") {
        ...
        errors.push("Invalid bundle: missing .gandalf/format-version".to_string());
    }
    ...
    let Some(manifest_entry) = manifest_entry else {
        errors.push("Invalid bundle: missing .gandalf/manifest.json".to_string());
    ...
    if let Some(checksums_entry) = entries.iter().find(|entry| entry.path == ".gandalf/checksums.json")
    {
        ...
        errors.push("Invalid bundle: missing .gandalf/checksums.json".to_string());
    }
...
fn resolve_signature_key(explicit_key: Option<&str>) -> Option<String> {
    explicit_key
        .map(str::to_string)
        .or_else(|| std::env::var("GANDALF_BUNDLE_KEY").ok())
}
...
fn canonical_signature_payload(entries: &[TarEntry], manifest: &BundleManifest) -> Vec<u8> {
    let mut hmac_entries: Vec<_> = entries
        .iter()
        .filter(|entry| {
            entry.path != ".gandalf/manifest.json" && entry.path != ".gandalf/checksums.json"
        })
...
fn update_manifest_entry(entries: &mut [TarEntry], manifest: &BundleManifest) -> BundleResult<()> {
    if let Some(index) = entries
        .iter()
        .position(|entry| entry.path == ".gandalf/manifest.json")
    {
