...
fn bundle_path(box_: &Sandbox, name: &str) -> PathBuf {
    box_.root.join(format!("{name}.gandalf"))
}
...
fn make_minimal_bundle(box_: &Sandbox, snapshot_name: &str, content_entries: Vec<TarEntry>) -> Vec<TarEntry> {
    ...
    let mut entries = vec![
        dir_entry(".gandalf/"),
        file_entry(".gandalf/format-version", b"1\n"),
        file_entry(
...
fn export_import_roundtrip_preserves_evidence() {
    ...
    assert!(export_result.bundle_path.ends_with(".gandalf"));
...
fn rejects_unsupported_format_version() {
    ...
    let entries = vec![
        dir_entry(".gandalf/"),
        file_entry(".gandalf/format-version", b"999\n"),
        file_entry(
            ".gandalf/manifest.json",
            format!("{}\n", serde_json::to_string(&manifest).unwrap()).as_bytes(),
        ),
...
fn dry_run_classifies_mcp_binaries_and_env_keys() {
    ...
    let npx = machine_diff
        ...
        .expect("npx report");
    assert_eq!(npx.binary_kind, Some(gandalf_core::McpBinaryKind::PackageRunner));
    ...
    assert_eq!(
        local.binary_kind,
        Some(gandalf_core::McpBinaryKind::SourceLocalPath)
    );
    ...
    assert!(result.readiness.summary[&gandalf_core::ReadinessCategory::NeedsManualAction] >= 2);
    assert_eq!(result.readiness.summary[&gandalf_core::ReadinessCategory::Unverified], 1);
    assert!(result.readiness.items.iter().any(|item| {
        item.code == "GANDALF_ENV_VALUE_REQUIRED" && item.problem.contains("OPENAI_API_KEY")
    }));
...
fn blocks_non_macos_content_apply_before_writing_files() {
    ...
    let error = bundle_import(&options).unwrap_err().to_string();
    assert!(error.contains("GANDALF_MACOS_APPLY_ONLY"));
...
fn dry_run_reports_non_macos_apply_blockers_without_throwing() {
    ...
    options.target_platform = Some("linux".to_string());
    let result = bundle_import(&options).expect("import");
    assert_eq!(result.readiness.summary[&gandalf_core::ReadinessCategory::Blocked], 1);
    assert_eq!(result.readiness.items[0].code, "GANDALF_MACOS_APPLY_ONLY");
...
fn dry_run_reports_non_macos_apply_limitations() {
    ...
    let result = bundle_import(&options).expect("import");
    assert_eq!(
        result.readiness.summary[&gandalf_core::ReadinessCategory::Unsupported],
        1
    );
    assert_eq!(result.readiness.items[0].code, "GANDALF_MACOS_APPLY_ONLY");
}
