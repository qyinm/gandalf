...
fn restores_codex_config_byte_for_byte_through_target_home() {
    ...
    let config_item = plan
        .items
        .iter()
        .find(|item| item.kind == gandalf_core::EvidenceKind::AgentConfig)
        .expect("config item");
...
fn deletes_codex_user_skill_added_after_baseline() {
    ...
    let skill_item = plan
        .items
        .iter()
        .find(|item| item.kind == gandalf_core::EvidenceKind::Skill && item.action == RestoreAction::Delete)
...
fn marks_codex_toml_mcp_changes_unsupported_while_config_carries_restore() {
    ...
    assert!(plan
        .items
        .iter()
        .any(|item| item.kind == gandalf_core::EvidenceKind::AgentConfig && item.action == RestoreAction::Update));
    assert!(plan.unsupported_items.iter().any(|item| {
        item.kind == gandalf_core::EvidenceKind::McpServer
...
fn rejects_restore_apply_outside_confinement_roots() {
    let (_root, project_path, home_dir, _store_dir) = make_restore_sandbox();
    let outside = "/etc/gandalf-restore-test-target";
    let mut item = make_item(
...
fn restore_plan_pipeline_applies_mcp_permission_and_env_with_confinement() {
    ...
    let mcp_item = plan
        .items
        .iter()
        .find(|item| item.kind == gandalf_core::EvidenceKind::McpServer)
    ...
    let permission_item = plan
        .items
        .iter()
        .find(|item| item.kind == gandalf_core::EvidenceKind::Permission)
    ...
    let env_item = plan
        .items
        .iter()
        .find(|item| item.kind == gandalf_core::EvidenceKind::EnvKey)
