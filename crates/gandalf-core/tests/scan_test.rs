...
fn discovers_project_mcp_and_reports_read_only_trust() {
    ...
    assert_eq!(github.restore_policy, gandalf_core::RestorePolicy::StructuredFieldsOnly);
...
fn emits_parse_failed_for_malformed_json() {
    ...
    assert!(scan.evidence.iter().any(|item| {
        item.source_path == ".claude/settings.json"
            && item.parser == gandalf_core::EvidenceParser::Json
...
fn captures_dotenv_key_inventory_while_omitting_secret_values() {
    ...
    assert!(env_evidence.iter().any(|item| {
        item.name.as_deref() == Some("GANDALF_MODE")
            && item.capture_status == CaptureStatus::Omitted
            && item.value.is_none()
    }));
...
"#,
    ...
    .expect("pi settings");
    fs::write(sandbox.project_path.join(".env"), "GANDALF_MODE=local\n").expect("env");
}

#[test]
fn export_scan_evidence_fixture_for_scratch() {
    let scratch_dir = match std::env::var("GANDALF_SCRATCH_DIR") {
        Ok(dir) => dir,
        Err(_) => return,
    };
    ...
    assert!(scan.evidence.iter().any(|item| {
        item.restore_policy == gandalf_core::RestorePolicy::StructuredFieldsOnly
            && item.kind == EvidenceKind::McpServer
    }));
    ...
    let output = serde_json::json!({
        "entrypoint": "gandalf_core::scan_project",
