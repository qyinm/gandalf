...
fn does_not_execute_path_hijacked_which_helper_during_command_lookup() {
    ...
    let fake_which = root.path().join("definitely-missing-hem-tool");
    ...
    let report = build_readiness_report(
        &[mcp_item(
            "mcp-missing",
            json!({ "command": "definitely-missing-hem-tool" }),
        )],
        &ReadinessOptions::default(),
    );
    ...
    assert!(report
        .items
        .iter()
        .any(|item| item.code == "HEM_MCP_COMMAND_MISSING"));
    assert!(!marker_path.exists());
}
...
fn reports_missing_env_keys_by_name_only() {
    ...
    let env_items: Vec<_> = report
        .items
        .iter()
        .filter(|entry| entry.code == "HEM_ENV_VALUE_REQUIRED")
        .collect();
...
fn treats_target_env_inventory_and_process_env_as_satisfying_env_keys() {
    ...
    assert!(!report
        .items
        .iter()
        .any(|entry| entry.code == "HEM_ENV_VALUE_REQUIRED"));
    let serialized = serde_json::to_string(&report).expect("serialize");
